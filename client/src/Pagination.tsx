...
const Pagination = ({ currentPage, pageCount, onPageChange }: { currentPage: number, pageCount: number, onPageChange: (page: number) => void}) => {

    //create an array of pages from 1 to pageCount
    const pages = Array.from({ length: pageCount }, (_, i) => i + 1).map((page) => page.toString());

    const renderPageNumber = (page: string) => {
        if(page == currentPage.toString()) {
            return (
                <a href="#" aria-current="page" className="px-3 py-2 text-blue-600 border border-gray-300 bg-blue-50 hover:bg-blue-100 hover:text-blue-700 dark:border-gray-700 dark:bg-gray-700 dark:text-white">{page}</a>
            )
        } else {
            return (
                <a href="#" className="px-3 py-2 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">{page}</a>
            )
        }
    }
    ...
    return (
        <nav aria-label="Page navigation example">
            <ul className="inline-flex -space-x-px">
                ...
                {renderPageNumber((pageCount - 1).toString())}
                <li>
